  ...
  return (
    <li key={key} className={cardStyles.card}>
      <p className={cardStyles.date}>
        <span className={cardStyles.cardDate}>{day}</span>
        <span className={cardStyles.cardMonthYear}>
          {month} {year}
        </span>
      </p>
      ...
      <div className={cardStyles.cardTags}>
        <Link as={`/categories/${data.category}`} href={`/categories/[slug]`}>
          <a className={cardStyles.cardCategory}>{category}</a>
        </Link>
        <Link as={`/authors/${data.author}`} href={`/authors/[slug]`}>
